...
const fs = require('fs')
const parameter = process.argv.slice(2); // major
...
function checkBrach(branch){
    ...
    if(branch !== 'develop'){
        console.log('\x1b[33m%s\x1b[0m', '⚠️  Atenção Processo deve executar na branch develop.')
        process.exit()
    }
}
...
const updateRelease = (version) => {
    ...
    if (parameter == "major") {
        ...
        newVersion = ([parseInt(arrayVersion[0]), parseInt(arrayVersion[1]) + 1, 0].concat()).join('.')
        
    } else if (parameter == "patch") {
        newVersion = ([parseInt(arrayVersion[0]), parseInt(arrayVersion[1]), parseInt(arrayVersion[2]) + 1].concat()).join('.')
    }
...
const readChangeLog = () => {
    ...
    const date = new Date();
    let ref = null
    const newLineChangeLog = `## [${version}] - ${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}`
    const tasks = []
    const lines = []
    try {
        const data = fs.readFileSync('./CHANGELOG.md', 'utf8').split(/\r?\n/);
            ...
            arr.forEach((line, idx)=> {
                ...
                }else if(ref!= null &&  ( line.startsWith('-') ||  line.startsWith("### Added") || line.startsWith("### Fixed") || line.startsWith("### Changed") || line.startsWith("### HotFix") ) ) {
